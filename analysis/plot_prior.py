...
def create_common_metric_plot(metric_key: str, plot_title: str, run_data: dict, colors: dict):
    ...
    output_filename_pdf = f"prior_factor_{plot_title.replace(' ', '_').lower()}_comparison.pdf"
    output_path_pdf = OUTPUT_DIR / output_filename_pdf
    fig.savefig(output_path_pdf, format="pdf", bbox_inches="tight")

    output_filename_svg = f"prior_factor_{plot_title.replace(' ', '_').lower()}_comparison.svg"
...
def plot_combined_gating_signals_quadrant(run_data: dict, colors: dict):
    ...
    output_path_pdf = OUTPUT_DIR / "prior_factor_combined_gating_signals.pdf"
    fig.savefig(output_path_pdf, format="pdf", bbox_inches="tight")

    output_path_svg = OUTPUT_DIR / "prior_factor_combined_gating_signals.svg"
    fig.savefig(output_path_svg, format="svg", bbox_inches="tight")
...
def plot_router_parameters_separately(run_data: dict, colors: dict):
    ...
    for param_col, param_title in params.items():
        ...
        output_filename_pdf = f"prior_factor_{param_col}_comparison.pdf"
        output_path_pdf = OUTPUT_DIR / output_filename_pdf
        fig.savefig(output_path_pdf, format="pdf", bbox_inches="tight")

        output_filename_svg = f"prior_factor_{param_col}_comparison.svg"
        output_path_svg = OUTPUT_DIR / output_filename_svg
        fig.savefig(output_path_svg, format="svg", bbox_inches="tight")
