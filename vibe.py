...
try:
        ...
        if hasattr(config, attr_name):
            ...
            if isinstance(current_value, list):
                if len(current_value) > 0:
                    ...
                    print(
                        f"WARNING: `{attr_name}` was a list ({current_value}), patched to: {patched_value}"
                    )
                    setattr(config, attr_name, patched_value)
                else:
                    print(
                        f"WARNING: `{attr_name}` was an empty list, setting to None/0. You might need to adjust this default."
                    )
                ...
                try:
                    if isinstance(current_value, bool):
                        ...
                        patched_value = int(current_value)
                    print(
                        f"WARNING: `{attr_name}` was type {type(current_value).__name__} ({current_value}), patched to int: {patched_value}"
                    )
                    setattr(config, attr_name, patched_value)
                    ...
                    try:
                        ...
                        print(
                            f"WARNING: `{attr_name}` was type {type(current_value).__name__} ({current_value}), patched to float: {patched_value}"
                        )
                        setattr(config, attr_name, patched_value)
                    except (ValueError, TypeError):
                        print(
                            f"WARNING: Could not convert `{attr_name}` ({current_value}) to int/float. Keeping original value. This might cause issues."
                        )
    ...
    if (
        ...
        if "type" not in config.rope_scaling:
            ...
            print(
                f"WARNING: Setting `rope_scaling.type` to be consistent with 'rope_type': {config.rope_scaling['rope_type']}."
            )
    else:
        if "type" not in config.rope_scaling:
            config.rope_scaling["type"] = "linear"
            print(
                "WARNING: `rope_scaling` lacks 'rope_type' (or it's None) and 'type'. Setting `type` to 'linear'."
            )
        if (
            ...
            print(
                f"WARNING: Setting `rope_scaling.rope_type` to be consistent with 'type': {config.rope_scaling['type']}."
            )

    if "factor" not in config.rope_scaling:
        config.rope_scaling["factor"] = 1.0
        print("WARNING: Config `rope_scaling` lacks 'factor'. Setting to 1.0.")
    ...
    print(
        "Please ensure the path is correct and the directory contains `config.json`, `pytorch_model.bin` (or `model.safetensors` files), and `tokenizer.json` (or similar)."
    )
...
input_encoding = tokenizer.encode_plus(
    prompt, return_tensors="pt", add_special_tokens=True, return_attention_mask=True
)
...
with torch.no_grad():
    ...
    ):  # device.split(':')[0] handles 'cuda:0' vs 'cuda'
...
generated_text = tokenizer.decode(
    generated_output[0, input_ids.shape[1] :], skip_special_tokens=True
)
...
print("\n" + "=" * 50)
print(
    "Vibe check complete! Experiment with different prompts and generation parameters."
)
print("=" * 50)
