...
def _log_gate_stats_for_benchmark(model, benchmark_name):
    ...
    if hasattr(model, "get_last_gate_means") and model._log_gates:
        ...
        if avg_gate_activation is not None:
            log.info(
                f"    Average gate activation for {benchmark_name}: {avg_gate_activation:.4f}"
            )
        else:
...
def run_perplexity_benchmark(model, tokenizer, config, num_samples, device):
    ...
    try:
        ...
        texts = _load_and_prepare_dataset(
            config, num_samples
    ...
    try:
        for subj in tqdm(config["subjects"], desc="MMLU subjects"):
            try:
                ...
                ds_dev = load_dataset(
                    config["dataset_id"], subj, split=config["dev_split"]
                )
                dev_shot_data = ds_dev.select(
                    range(min(config["num_shot"], len(ds_dev)))
                )

                # Apply preprocessing for each subject's test set with the few-shot data
                # Pass dev_shot_data directly to preprocess_fn
                processed_ds = ds_test.map(
                    lambda ex: config["preprocess_fn"](ex, dev_shot_data)
                )
        ...
        if mmlu_accs:
            ...
            if model_gate_means_mmlu:
                all_layer_activations = [
                    item for sublist in model_gate_means_mmlu for item in sublist
                ]
                overall_mmlu_gate_activation = (
                    sum(all_layer_activations) / len(all_layer_activations)
                    if all_layer_activations
                    else None
                )
                if overall_mmlu_gate_activation is not None:
                    log.info(
                        f"    Average gate activation for MMLU_mean: {overall_mmlu_gate_activation:.4f}"
                    )

            return {
                "mean_accuracy": mean_mmlu,
                "individual_accuracies": {
                    s: a for s, a in zip(config["subjects"], mmlu_accs)
                },
            }
        else:
        ...
        if hasattr(model, "enable_gate_logging") and model._log_gates:
            ...
            model._last_gate_means = (
                None  # Reset global gate stats after this multi-stage benchmark
            )
