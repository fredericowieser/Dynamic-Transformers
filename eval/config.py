...
class EvalConfig:
    ...
    def load_from_model(self, model_path: str):
        ...
        try:
            ...
            cfg = hydra.utils.load_config_from_file(
                os.path.join(model_path, "config.yaml")
            )
            self.ce_bias = (
                cfg.get("model", {}).get("model_cfg", {}).get("ce_bias", self.ce_bias)
            )
            self.dynamic_k = (
                cfg.get("model", {})
                .get("model_cfg", {})
                .get("dynamic_k", self.dynamic_k)
            )
        except Exception as e:
            print(
                f"Warning: Could not load from model config. Using defaults. Error: {e}"
            )
