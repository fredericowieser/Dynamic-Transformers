...
def compute_ppl_from_texts(
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="PPL batches"):
        ...
        if out.loss is not None:
            ...
            log.warning(
                f"Model output loss was None for a batch during PPL computation. Skipping this batch."
            )
...
def mc_accuracy(
    ...
    if hasattr(model, "enable_gate_logging") and model._log_gates:
        ...
        def wrapped_forward(*f_args, **f_kwargs):
            ...
            if (
                hasattr(model, "_log_gates")
                and model._log_gates
                and hasattr(model, "get_last_gate_means")
            ):
    ...
    try:
        for ex in tqdm(dataset, desc="MC eval"):
            ...
            for i, choice_text in enumerate(choices):
                if (
                    not isinstance(choice_text, str) or not choice_text.strip()
                ):  # Skip non-string or empty choices
                    continue
                ...
                if is_instruct:
                    ...
                    prompt_only_text_for_len = tokenizer.apply_chat_template(
                        prompt_only_conv, tokenize=False, add_generation_prompt=True
                    )
                    prompt_len = tokenizer(
                        prompt_only_text_for_len, return_tensors="pt"
                    ).input_ids.shape[-1]
                    ...
                    full_encoded = tokenizer(full_text, return_tensors="pt").to(
                        model.device
                    )
                    ...
                    prompt_encoded = tokenizer(prompt + " ", return_tensors="pt").to(
                        model.device
                    )
                ...
                if input_ids.shape[1] <= prompt_len:
                    ...
                    with torch.no_grad():
                        ...
                        choice_logprobs_all_vocab = logprobs[
                            0, prompt_len - 1 : -1
                        ]  # Logprobs for all vocab at each position that *predicts* a choice token
                        choice_ids_actual = input_ids[
                            0, prompt_len:
                        ]  # Actual token IDs for the choice text
                        ...
                        if choice_ids_actual.numel() == 0:
                            ...
                            gathered_logprobs = torch.gather(
                                choice_logprobs_all_vocab,
                                dim=1,
                                index=choice_ids_actual.unsqueeze(-1),
                            ).squeeze(
                                -1
                            )  # (num_choice_tokens,)
        ...
        if hasattr(model, "enable_gate_logging") and model._log_gates:
            ...
            if model_gate_means_history:
                ...
                all_layer_activations = [
                    item for sublist in model_gate_means_history for item in sublist
                ]
                if all_layer_activations:
                    model._last_gate_means = sum(all_layer_activations) / len(
                        all_layer_activations
                    )
                else:
...
def generative_exact_match(
    gen_pipe_obj: pipeline, dataset, question_key, context_key, answers_key
):
    ...
    try:
        for ex in tqdm(dataset, desc="Generative EM/F1"):
            prompt_text = (
                f"Question: {ex[question_key]}\nContext: {ex[context_key]}\nAnswer:"
            )
            # Use the pipeline object passed in
            pred = (
                gen_pipe_obj(prompt_text, max_new_tokens=32, do_sample=False)[0][
                    "generated_text"
                ]
                .split("Answer:")[-1]
                .strip()
