...
from transformers.models.qwen2.modeling_qwen2 import Qwen2RMSNorm, Qwen2RotaryEmbedding, Qwen2DecoderLayer
from transformers.modeling_attn_mask_utils import _prepare_4d_causal_attention_mask
from .block import DynamicBlock
...
class BaseSurpriseRouter(BaseRouter, ABC):
    ...
    def __init__(self, config, capacity_attr: str):
        ...
        self.ma_window = int(config.params.ma_window)
    ...
    def _get_vpr_signals(self, D_st, D_ch, beta_ce, beta_cu):
        ...
        return g_cont, {"S_CE_mean": S_CE.mean().item(), "S_CU_mean": S_CU.mean().item()}
...
class MoDRouter(BaseRouter):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.aux_loss_weight = getattr(config.params, 'mod_aux_loss_weight', 0.01)
...
class SDTForCausalLM(BaseForCausalLM):
    ...
    def _forward_layers(self, hidden_states, **kwargs):
        ...
        for layer_pair in self.layers:
            ...
            scores, _, _ = router(**decision_output, **kwargs)
...
class SDTRouter(BaseSurpriseRouter):
    ...
    def forward(self, original: torch.Tensor, posterior: torch.Tensor, prior: torch.Tensor, beta_ce: float, beta_cu: float, **kwargs) -> Tuple[torch.Tensor, Optional[torch.Tensor], Dict[str, Any]]:
...
class STTForCausalLM(BaseForCausalLM):
    ...
    def _forward_layers(self, hidden_states, **kwargs):
        ...
        for layer in self.layers:
            if isinstance(layer, STTLayer):
                hidden_states, aux_loss, _ = layer(hidden_states, **kwargs)
    ...
    def get_trainable_parameters(self):
        # Keywords match the sub-modules within STTLayer
...
class STTLayer(nn.Module):
    ...
    def forward(self, hidden_states, **kwargs):
        ...
        if self.training:
            ...
            _, binary_targets, _ = self.predictive_router(
                actual_residual, predicted_residual, **kwargs
            )
            
            causal_logits, _, _ = self.causal_router(original_hidden)
            causal_loss = F.binary_cross_entropy_with_logits(causal_logits, binary_targets.detach())
            
            aux_loss = (self.config.params.tpn_loss_weight * tpn_loss) + \
                       (self.config.params.causal_loss_weight * causal_loss)
        ...
        return final_hidden_states, aux_loss, {} # Return empty stats dict for now
```
...
import os
import torch
import loggingrom pathlib import Path
...
def evaluate_perplexity(model, dataloader, accelerator):
    """A simplified evaluation function that only calculates validation loss and perplexity."""
    model.eval()
    losses = []
    ...
    for epoch in range(cfg.training.num_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            if (global_step) % cfg.training.eval_interval == 0:
                ...
                val_loss, val_perplexity = evaluate(unwrapped_model, eval_loader, accelerator, cfg, tokenizer)
