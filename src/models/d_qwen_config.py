...
class DynamicQwenConfig(Qwen2Config):
    ...
    def __init__(self, **kwargs):
        ...
        self.capacity_gamma = kwargs.pop(
            "capacity_gamma", 1.0
        )  # float in (0,1], fraction of tokens to compute (like MoD's k)
        self.beta_ce_init = kwargs.pop(
            "beta_ce_init", 1.0
        )  # Initial beta for CE sigmoid
        self.beta_cu_init = kwargs.pop(
            "beta_cu_init", 1.0
        )  # Initial beta for CU sigmoid
        self.cu_detection_multiplier_init = kwargs.pop(
            "cu_detection_multiplier_init", 1.0
        )  # Initial multiplier for CU norm
        self.ce_criterion_offset_init = kwargs.pop(
            "ce_criterion_offset_init", 0.0
        )  # Offset for CE criterion (replaces old ce_bias concept)

        self.token_wise_gating = kwargs.pop(
            "token_wise_gating", True
        )  # Whether to apply gating per-token or per-batch
        self.moving_average_window_size = kwargs.pop(
            "moving_average_window_size", 100
        )  # Window size for D_st moving average in CU
        ...
        self.prior_ffn_intermediate_size_factor = (
            self.prior_ffn_intermediate_size_factor
        )
