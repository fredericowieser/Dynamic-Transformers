...
class DynamicLlamaForCausalLM(LlamaForCausalLM):
    ...
    def _modify_model_architecture(self):
        ...
        for i, layer in enumerate(self.model.layers):
            ...
            if init_prior_from_mlp_flag:
                ...
                original_mlp_state_dict = {
                    k: v.cpu() for k, v in original_mlp_state_dict.items()
                }
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        init_prior_from_mlp_flag = kwargs.get(
            "init_prior_from_mlp", getattr(model.config, "init_prior_from_mlp", False)
        )
        ...
        for i, layer in enumerate(model.model.layers):
            ...
            new_layers.append(custom_layer)
            log.info(
                f"Successfully re-instantiated layer {i} as DynamicLlamaDecoderLayer for loading."
            )
        model.model.layers = new_layers

        model.dynamic_k = kwargs.get("dynamic_k", model.config.dynamic_k)
        model.ce_bias = kwargs.get("ce_bias", model.config.ce_bias)
        model.gate_warmup_iters = kwargs.get(
            "gate_warmup_iters", model.config.gate_warmup_iters
        )

        model.enable_lora_main_path = kwargs.get(
            "enable_lora_main_path",
            getattr(model.config, "enable_lora_main_path", False),
        )
        model.enable_lora_prior_ffn = kwargs.get(
            "enable_lora_prior_ffn",
            getattr(model.config, "enable_lora_prior_ffn", False),
        )
