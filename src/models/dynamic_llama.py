...
class DynamicLlamaBlockWiseDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        d_ch_tok = F.mse_loss(
            posterior_full_path_output, prior_prediction, reduction="none"
        ).mean(
            -1
        )  # (B, T)
        ...
        gate_vec = (
            CE | CU
        ).float()  # (B,) - 1.0 means activate posterior, 0.0 means activate original input
        ...
        hidden_states_final = (
            gate * posterior_full_path_output + (1.0 - gate) * original_input_to_block
        )  # (B, T, C)

        # Prediction-loss for the prior FFN
        prior_loss = F.mse_loss(
            prior_prediction, posterior_full_path_output.detach()
        )  # Scalar
