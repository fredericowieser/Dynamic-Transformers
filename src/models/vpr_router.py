...
from torch import (
    nn,
)  # Keep nn imported if you use other nn.Modules like nn.Linear, etc.
...
class VPRRouter(nn.Module):
    ...
    def forward(
        ...
        d_st_tok = F.mse_loss(
            posterior_full_path_output, original_input_to_block, reduction="none"
        ).mean(
            -1
        )  # (B, T)
        d_ch_tok = F.mse_loss(
            posterior_full_path_output, prior_hidden_states, reduction="none"
        ).mean(
            -1
        )  # (B, T)
        ...
        if self.token_wise_gating:
            ...
            ma_d_st_tok = self._calculate_moving_average(
                d_st_tok.detach()
            )  # Detach for stable moving average.
            # --- START OF CHANGE ---
            CU_val = d_st_tok - (
                self.cu_detection_multiplier_val * ma_d_st_tok
            )  # Use the fixed value
            # --- END OF CHANGE ---
        else:
            ...
            # --- START OF CHANGE ---
            CU_val = mean_d_st - (
                self.cu_detection_multiplier_val * mean_d_st.detach()
            )  # Use the fixed value
            # --- END OF CHANGE ---
        ...
        if capacity_gamma >= 1.0:  # If gamma is 1.0, process all tokens.
            threshold = -torch.finfo(
                combined_gating_signal_continuous.dtype
            ).max  # Always pass
        else:
