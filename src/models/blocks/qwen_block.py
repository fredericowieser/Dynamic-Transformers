...
class Qwen2Block(nn.Module):
    ...
    def forward(
        ...
        # This logic is required by Qwen2Attention to compute rotary embeddings
        input_for_rope = hidden_states_norm.view(
            batch_size, seq_len, num_attention_heads, head_dim
        ).transpose(1, 2)
        
        cos, sin = self.self_attn.rotary_emb(input_for_rope, seq_len=position_ids.max() + 1)
        ...
        attn_outputs = self.self_attn(
            ...
            use_cache=use_cache,
            position_embeddings=position_embeddings, # Pass the required argument
        )
