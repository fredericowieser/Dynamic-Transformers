...
def _sparse_bwd_preprocess(
    ...
    o = tl.load(O_ptr, mask=mask_md, other=0.0).to(tl.float32)
    do = tl.load(dO_ptr, mask=mask_md, other=0.0).to(tl.float32)

    d = tl.sum(o * do, axis=1)
    tl.store(D_ptr, d, mask=mask_m)
...
def _sparse_bwd_kernel(
    ...
    do = tl.load(do_ptr, mask=mask_md, other=0.0)
    m = tl.load(m_ptr, mask=mask_m, other=0.0)
    d = tl.load(d_ptr, mask=mask_m, other=0.0)
    q_real_pos = tl.load(idx_ptr, mask=mask_m, other=-1)
...
class SparseCausalAttention(torch.autograd.Function):
    ...
    def forward(ctx, q_selected, k_full, v_full, real_indices, sm_scale):
        ...
        grid = (triton.cdiv(N_SEL, 64), B * H)

        _sparse_fwd_kernel[grid](
            q, k, v, sm_scale, M, o, real_indices,
