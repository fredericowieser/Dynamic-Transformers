...
def _patch_rope_scaling(config):
    ...
    if rs is None or not isinstance(rs, dict):
        print(f"INFO: Patching missing/invalid rope_scaling (was {rs!r}) -> {{'type':'linear', ...}}",
              file=sys.stderr)
    ...
    if "rope_type" not in rs or rs["rope_type"] is None:
        print(f"INFO: Patching rope_scaling['rope_type'] from {rs.get('rope_type')!r} -> {rs['type']!r}",
              file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--dynamic_k", type=float, default=None,
                        help="Override the model's default dynamic_k.")
    parser.add_argument("--ce_bias", type=float, default=0.0,
    ...
    if not isinstance(model, DynamicLlamaForCausalLM):
        ...
        print("Make sure you `pip install -e .` so that `trust_remote_code` finds your class.", file=sys.stderr)
        sys.exit(1)
